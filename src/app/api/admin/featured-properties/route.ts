// src/app/api/admin/featured-properties/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const featuredProperties = featuredResult.rows;

    // Group featured properties with their images
    ...
    featuredProperties.forEach(row => {
      if (!featuredMap.has(row.id)) {
        ...
        featuredMap.get(row.id).images.push(row.image);
      }
    });

    const featuredList = Array.from(featuredMap.values());
    ...
    const allProperties = allResult.rows;
